...
resume_execute_a_thread:                          ; Resuming execution of a thread
         ...
         mov rbx, [r11 + 56]
         mov cr3, rbx                             ;切换地址空间 Switch address space
...
create_process:                                   ; Create a new task and its main thread
         ...
         ; Because we are executing at the high end of the address space, we can execute kernel code and access kernel data normally, after all, all
         ; the high-end (global) parts of the tasks are the same. At the same time, the stack currently in use is the stack at the high end of the address space.
         ; The high-end stack of the task is the same as the stack currently in use. So, we can switch to the new task's address space and clear the first half of its 4-level header table.
...
init:    ; Initialization of the working environment of the kernel
  ...
  .ioapic:
   ...
   .m_end:
         ...
         mov dword [rsi + 0x320], 0x10000         ; shot）模式 Local vector table entry register of the timer. Single-shot mode
