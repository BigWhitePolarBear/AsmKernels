...
no_ia_32e:
	...
	pge:
		...
		; Create a 4-level header entry corresponding to the lower 2MB of memory corresponding to the 
		; 4-level header entry of the linear address range: 0x0000000000000000--0x00000000001FFFFF
		; This entry ensures that the lower 2MB of physical memory (including the kernel) can be
		; accessed normally after paging is enabled and before it is mapped to the high end.
		...
		; Create an entry in the page directory pointer table corresponding to the lower 2MB of memory corresponding to the
		; entry of the linear address range: 0x0000000000000000--0x00000000001FFFFF
		...
		; Create an entry in the page directory table corresponding to the lower 2MB of memory corresponding to the
		; entry of the linear address range: 0x0000000000000000--0x00000000001FFFFF
		mov dword [ebx + 0 * 8], 0 | 0x83			; Physical address and attributes of the 2MB page
		mov dword [ebx + 0 * 8 + 4], 0


		; Create a 4-level header entry corresponding to the linear address range 0xFFFF800000000000--0xFFFF8000001FFFFF 
		; in the 4-level header table, and map the kernel to the high end. The kernel should work in the high end of the
		; linear address after entering IA-32e mode.
